...
import { useGetAllPartiesPayablesLeftQuery } from "../../redux/api/partyAPi";




export default function PartyPayablesLeftOld() {
...
//   const { data: allPartiesPayablesLeft, isLoading } = 
...
//     toDate,
//   });
  //console.log(allPartiesPayablesLeft);

  // const[selecedallPartiesReceivablesLeft,setSelectedallPartiesReceivablesLeft]= useState(null);
const{data:allPartiesPayablesLeft,isLoading:isLoadingallPartiesPayablesLeft}=
useGetAllPartiesPayablesLeftQuery({
  page,
  search: searchTerm,
  fromDate,
  toDate,
});
// const allPartiesPayablesLeft=allPartiesReceivablesLeftQuery?.data||[];
console.log(allPartiesPayablesLeft);
 ...
 setSearchParams({
  ...
  //console.log(allPartiesPayablesLeft?.allPartiesPayablesLeft);
    ...
    <>
            ...
            <div className="box-inn-sp">
              ...
              <div className="inn-title">
                <div className="flex flex-col sm:flex-col lg:flex-row justify-between lg:items-center">
                  ...
                  <div className="flex flex-row justify-between items-center mb-4 sm:mb-4">
                    <div>
                      <h4 className="text-2xl font-bold mb-1">All Parties Payables Left</h4>
                      <p className="text-gray-500 text-sm sm:text-base">
                        All Parties Payables Details
                      </p>
                    </div>
                ...
                {/* <div className="flex flex-col bg-white p-6 rounded-xl shadow-md w-full max-w-sm"> */}
                  ...
                  {/* Total allPartiesPayablesLeft */}
                  {/* <div className="mb-2 text-left">
                    <p className="text-sm font-medium text-black">Total allPartiesPayablesLeft Amount</p>
                    <h4 className="text-3xl font-bold text-black">₹ {allPartiesPayablesLeft?.totals?.totalAmount}</h4>
                  </div> */}
                  ...
                  <div className=" flex flex-col gap-2 sm:flex-row sm:-gap-4">
                    <div className="flex  ">
                      ...
                      <span className="text-sm font-semibold text-black">₹ {allPartiesPayablesLeft?.totals?.totalReceived}</span>
                    </div>

                    <div className="flex">
                      <span className="text-sm font-medium text-gray-500">Balance Due &nbsp; &nbsp;</span>
                      <span className="text-sm font-semibold text-black">₹ {allPartiesPayablesLeft?.totals?.totalBalance}</span>
                    </div>
                  </div> */}
              ...
              <div className="tab-inn">
                <div className="table-responsive table-desi">
                  {isLoadingallPartiesPayablesLeft ? (
                    <p className="text-center mt-4">Fetching all Parties PayablesLeft...</p>
                  ) : allPartiesPayablesLeft?.length === 0 ? (
                    <p className="text-center mt-4">No all Parties PayablesLeft found.</p>
                  ) : (
                    ...
                    <table className="w-full min-w-[500px]">
                      ...
                      <tbody>
                        {allPartiesPayablesLeft && allPartiesPayablesLeft?.data?.length > 0 ? (
                          allPartiesPayablesLeft?.data?.map((purchase, idx) => (
                            <tr key={purchase?.Purchase_Id}>
                              <td>
                                {(allPartiesPayablesLeft?.currentPage - 1) * 10 + (idx + 1)}.
                              </td>
                              <td >
                                {purchase?.Bill_Date
                              ...
                              <td style={{color: "red"}}>{purchase?.Balance_Due || "N/A"}</td>
      ...
      `}
        ...
        >
          {index + 1}
                    ...
                    {(() => {
                      const totalPages = allPartiesPayablesLeft?.totalPages || 1;
                  ...
                  <div className="sm:hidden px-3 py-1 bg-gray-100 rounded text-sm">
                    Page {page} / {allPartiesPayablesLeft?.totalPages || 1}
                  </div>
                  ...
                  <button
                    ...
                    disabled={page === allPartiesPayablesLeft?.totalPages ||
                      allPartiesPayablesLeft?.totalPages === 0}
                    className={`px-3 py-1 bg-gray-200 hover:bg-gray-300 rounded
        ${page === allPartiesPayablesLeft?.totalPages ||
                        allPartiesPayablesLeft?.totalPages === 0
                        ? 'opacity-50 '
                        : ''
                      }
      `}
              ...
              {/* <div className="flex justify-center align-center space-x-2 p-4">
                ...
                {[...Array(allPartiesPayablesLeft?.totalPages).keys()].map((index) => (
                  <button
                    key={index}
                    onClick={() => handlePageChange(index + 1)}
                ...
                <button type="button"
                  onClick={() => handleNextPage()}
                  disabled={page === allPartiesPayablesLeft?.totalPages || allPartiesPayablesLeft?.totalPages === 0}
                  className={`px-3 py-1 bg-gray-200 hover:bg-gray-300 rounded
                ${page === allPartiesPayablesLeft?.totalPages || allPartiesPayablesLeft?.totalPages === 0 ? 'opacity-50 ' : ''}
                `}
                >
                  Next →
                </button>
