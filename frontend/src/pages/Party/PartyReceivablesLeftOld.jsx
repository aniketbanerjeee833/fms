...
import { useGetAllPartiesReceivablesLeftQuery } from "../../redux/api/partyAPi";


export default function PartyReceivablesLeftOld() {
...
//   const { data: allPartiesReceivablesLeft, isLoading } = 
// useGetAllallPartiesReceivablesLeftQuery({
//     page,
//     search: searchTerm,
//     fromDate,
//     toDate,
//   });
  //console.log(allPartiesReceivablesLeft);

  // const[selecedallPartiesReceivablesLeft,setSelectedallPartiesReceivablesLeft]= useState(null);
const{data:allPartiesReceivablesLeft,isLoading:isLoadingallPartiesReceivableLeft}=
useGetAllPartiesReceivablesLeftQuery({
  page,
  search: searchTerm,
  fromDate,
  toDate,
});
// const allPartiesReceivablesLeft=allPartiesReceivablesLeftQuery?.data||[];
console.log(allPartiesReceivablesLeft);
 ...
 setSearchParams({
  ...
  //console.log(allPartiesReceivablesLeft?.allPartiesReceivablesLeft);
              ...
              <div className="inn-title">
                <div className="flex flex-col sm:flex-col lg:flex-row justify-between lg:items-center">
                  ...
                  <div className="flex flex-row justify-between items-center mb-4 sm:mb-4">
                    <div>
                      <h4 className="text-2xl font-bold mb-1">All Parties Receivables Left</h4>
                      <p className="text-gray-500 text-sm sm:text-base">
                        All Parties Receivables Details
                      </p>
                    </div>
...
{/* 
                ...
                {/* <div className="flex flex-col bg-white p-6 rounded-xl shadow-md w-full max-w-sm"> */}
                  ...
                  {/* Total allPartiesReceivablesLeft */}
                  {/* <div className="mb-2 text-left">
                    <p className="text-sm font-medium text-black">Total allPartiesReceivablesLeft Amount</p>
                    <h4 className="text-3xl font-bold text-black">₹ {allPartiesReceivablesLeft?.totals?.totalAmount}</h4>
                  </div> */}
                  ...
                  <div className=" flex flex-col gap-2 sm:flex-row sm:-gap-4">
                    <div className="flex  ">
                      ...
                      <span className="text-sm font-semibold text-black">₹ {allPartiesReceivablesLeft?.totals?.totalReceived}</span>
                    </div>

                    <div className="flex">
                      <span className="text-sm font-medium text-gray-500">Balance Due &nbsp; &nbsp;</span>
                      <span className="text-sm font-semibold text-black">₹ {allPartiesReceivablesLeft?.totals?.totalBalance}</span>
                    </div>
                  </div> */}
              ...
              <div className="tab-inn">
                <div className="table-responsive table-desi">
                  {isLoadingallPartiesReceivableLeft ? (
                    <p className="text-center mt-4">Fetching allPartiesReceivablesLeft...</p>
                  ) : allPartiesReceivablesLeft?.length === 0 ? (
                    <p className="text-center mt-4">No allPartiesReceivablesLeft found.</p>
                  ) : (
                    ...
                    <table className="w-full min-w-[500px]">
                      ...
                      <tbody>
                        {allPartiesReceivablesLeft && allPartiesReceivablesLeft?.data?.length > 0 ? (
                          allPartiesReceivablesLeft?.data?.map((sale, idx) => (
                            <tr key={sale?.Sale_Id}>
                              <td>
                                {(allPartiesReceivablesLeft?.currentPage - 1) * 10 + (idx + 1)}.
                              </td>
                              <td >
                                {sale?.Invoice_Date
                              ...
                              <td style={{color: "green"}}>{sale?.Balance_Due || "N/A"}</td>
      ...
      `}
        ...
        >
          {index + 1}
                    ...
                    {(() => {
                      const totalPages = allPartiesReceivablesLeft?.totalPages || 1;
                  ...
                  <div className="sm:hidden px-3 py-1 bg-gray-100 rounded text-sm">
                    Page {page} / {allPartiesReceivablesLeft?.totalPages || 1}
                  </div>
                  ...
                  <button
                    ...
                    disabled={page === allPartiesReceivablesLeft?.totalPages ||
                      allPartiesReceivablesLeft?.totalPages === 0}
                    className={`px-3 py-1 bg-gray-200 hover:bg-gray-300 rounded
        ${page === allPartiesReceivablesLeft?.totalPages ||
                        allPartiesReceivablesLeft?.totalPages === 0
                        ? 'opacity-50 '
                        : ''
                      }
      `}
              ...
              {/* <div className="flex justify-center align-center space-x-2 p-4">
                ...
                {[...Array(allPartiesReceivablesLeft?.totalPages).keys()].map((index) => (
                  <button
                    key={index}
                    onClick={() => handlePageChange(index + 1)}
                ...
                <button type="button"
                  onClick={() => handleNextPage()}
                  disabled={page === allPartiesReceivablesLeft?.totalPages || allPartiesReceivablesLeft?.totalPages === 0}
                  className={`px-3 py-1 bg-gray-200 hover:bg-gray-300 rounded
                ${page === allPartiesReceivablesLeft?.totalPages || allPartiesReceivablesLeft?.totalPages === 0 ? 'opacity-50 ' : ''}
                `}
                >
                  Next →
                </button>
