...
//   // const recomputeTotals = (rawTotal, roundOffEnabled) => {
//   //   const rounded = Math.round(rawTotal);
//   //   const diff = rounded - rawTotal; // e.g. raw 240.40 -> diff -0.40 ; raw 239.60 -> diff +0.40

//   //   if (roundOffEnabled) {
//   //     setValue("Total_Amount", rounded.toFixed(2), { shouldValidate: true, shouldDirty: true });
//   //     setValue("Round_Off", diff.toFixed(2), { shouldValidate: true, shouldDirty: true });
//   //   } else {
//   //     setValue("Total_Amount", rawTotal.toFixed(2), { shouldValidate: true, shouldDirty: true });
//   //     setValue("Round_Off", diff.toFixed(2), { shouldValidate: true, shouldDirty: true }); // shown but not applied
//   //   }

//   //   return { rounded, rawTotal, diff };
//   // };
//   // const recalculateAllTotals = (roundOffEnabled) => {
//   //   //const currentItems = watch("items") || [];
//   //   const rawTotal = itemsValues.reduce((sum, it) => sum + (Number(it.Amount) || 0), 0);

//   //   recomputeTotals(rawTotal, roundOffEnabled);

//   //   const finalTotal = roundOffEnabled ? Math.round(rawTotal) : rawTotal;
//   //   setValue("Balance_Due", (finalTotal - Number(watch("Total_Paid") || 0)).toFixed(2), {
//   //     shouldValidate: true,
//   //     shouldDirty: true,
//   //   });
//   // };
...
export default function PurchaseAdd() {
  ...
  // const [allDropdownItems, setAllDropdownItems] = useState([]);
  // const [itemsHasMore, setItemsHasMore] = useState(false);
  // const [itemsNextCursor, setItemsNextCursor] = useState(null);
  const [activeItemRow, setActiveItemRow] = useState(null);
  ...
  // Keep the existing discount percentage
  const discountPercentage =
    Number(watch("Transaction_Discount_Percentage")) || 0;

  // Recalculate discount amount based on current item total
  const discountAmount =
  ...
  if (isRoundOff) {
    ...
    roundOff = Number(
      (rounded - afterDiscount).toFixed(2)
    );
