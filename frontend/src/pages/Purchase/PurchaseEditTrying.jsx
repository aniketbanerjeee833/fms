...
export default function PurchaseEdit() {
  ...
  const [hasSeenBillingNameParty, setHasSeenBillingNameParty] =
    useState(false);
  const [initialBillHadBillingName, setInitialBillHadBillingName] =
    useState(false);
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (!value.trim()) {
                                ...
                                if (initialBillHadBillingName && !hasSwitchedParty) {
                                  // Case 1: first empty/change
                                  setShowBillingName(false);
                                } else if (hasSeenBillingNameParty) {
                                  // Billing Name has already been encountered
                                  // → keep field visible
                                  setShowBillingName(true);
                                } else {
                                  // Case 2: still haven't encountered Billing Name
                                  setShowBillingName(false);
                                }
                              ...
                              if (matchedParty) {
                                ...
                                if (matchedParty.Billing_Name?.trim()) {
                                  ...
                                  // Billing Name has now been encountered
                                  setHasSeenBillingNameParty(true);
                                } else if (
                                  hasSeenBillingNameParty &&
                                  enableBillingNameOfParties
                                ) {
                                  // Billing Name was encountered earlier,
                                  // so use Party Name as Billing Name
                                  setValue(
                                    "Billing_Name",
                                    matchedParty.Party_Name,
                                    {
                                  ...
                                  // No Billing Name encountered yet
                                  setValue("Billing_Name", "", {
                                    shouldValidate: true,
                                    shouldDirty: true,
                                  });
                                ...
                                // IMPORTANT:
                                // Do NOT set hasSwitchedParty(true) here.
                                if (
                                  matchedParty.Party_Name?.trim().toLowerCase() !==
                                  purchase?.billPurchaseDetails?.Party_Name?.trim().toLowerCase()
                                ) {
                                  setHasSwitchedParty(true);
                                }
                              }
                              ...
                              // =====================================================
                                ...
                                if (
                                  ...
                                  // CASE 1:
                                  // Bill originally had Billing Name.
                                  // First mismatch → CLEAR + HIDE
                                  ...
                                  //
                                  // In both cases:
                                  // CLEAR VALUE + KEEP FIELD VISIBLE

                                  setShowBillingName(true);
                                } else {
                                  // CASE 2:
                                  // Bill initially had no Billing Name
                                  // and we have not encountered one yet.

                                  setShowBillingName(false);
                                }

                                // First mismatch/change has now happened
                                setHasSwitchedParty(true);
                              }
                            }}
                            onClick={(e) => {
                            ...
                            onBlur={() => {
                              setTimeout(() => {
                                ...
                                if (matchedParty) {
                                  ...
                                  // setHasSwitchedParty(true);
                                  if (initialBillHadBillingName && !hasSwitchedParty) {
                                    // Case 1: first mismatch
                                    setShowBillingName(false);
                                  } else if (hasSeenBillingNameParty) {
                                    // Subsequent mismatch
                                    // OR Case 2 after Billing Name was encountered
                                    setShowBillingName(true);
                                  } else {
                                    // Case 2 before Billing Name is encountered
                                    setShowBillingName(false);
                                  }
  ...
  <PartyAddModal
    ...
    onSave={(newParty) => {
      ...
      if (newParty.Billing_Name?.trim()) {
        // New party was created with a Billing Name
        ...
        // Do NOT use Party_Name immediately.
        setValue("Billing_Name", "", {
          shouldValidate: true,
          shouldDirty: true,
        });

        // If Billing Name section is enabled/visible,
        // keep the field visible but empty.
        setShowBillingName(enableBillingNameOfParties);
      }

      // New party means we have switched from the original party.
      setHasSwitchedParty(true);
