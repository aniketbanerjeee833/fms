...
export const purchaseFormSchema = z
 ...
 splits: z
  ...
  .array(
    z.object({
      // 🔹 Category — optional, defaults to "All" if not picked
      Item_Category: z.string().optional().default("All"),

      Item_Name: z.string().min(1, "Item name is required"),

      // 🔹 HSN — optional now, no min/max/regex enforcement unless a value is typed
      ...
      // 🔹 Unit — optional, no forced selection
      Item_Unit: z.string().optional().default(""),

      // 🔹 Purchase Price — optional, defaults to 0, no >0 refine anymore
