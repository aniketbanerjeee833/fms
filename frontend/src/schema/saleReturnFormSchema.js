...
const paymentSplitSchema = z
  .object({
    ...
    Amount: digitsOnly("Amount", false), // 🔹 required: false
  })
...
/* ─────────────────────────────────────────────────────────────
   SALE RETURN SCHEMA — fully lenient, mirrors purchaseReturnFormSchema
───────────────────────────────────────────────────────────────*/
export const saleReturnFormSchema = z.object({
  /* ── Party ── */
  ...
  /* ── Original invoice reference — optional, blank still saves ── */
  ...
  // 🔹 Total_Paid — money going back out to the customer on a sale return
  Total_Paid: z.string().optional().or(digitsOnly("Total_Paid", false)),

  /* ── splits — optional, no forced entry if amount is 0 ── */
  splits: z
    .array(paymentSplitSchema)
    .optional()
    .default([])
  ...
  /* ── items — allowed to be empty array entirely ── */
  items: z
    .array(
      z.object({
