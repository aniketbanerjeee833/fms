...
export default function TermsConditionsModal({ mode = "add", initialData = null, onClose, onSave, isSaving = false }) {
  ...
  return (
    ...
    >
      {/* 🔹 flex column + overflow-hidden + fixed max-h so header/footer never get pushed off-screen —
          only the middle "flex-1 overflow-y-auto" section scrolls, the modal shell itself never grows past 90vh */}
      <div
        className="bg-white w-full max-w-2xl rounded-lg shadow-lg flex flex-col"
        style={{ maxHeight: "90vh" }}
      >
        ...
        {/* Body — the ONLY scrollable region. min-h-0 is required for flex children to actually shrink/scroll instead of overflowing their parent */}
        <div className="flex flex-col flex-1 overflow-y-auto px-6 py-4 gap-5" style={{ minHeight: 0 }}>
          {/* Title — small */}
          <div className="flex flex-col gap-1 sm:w-1/2">
            <span className="active">Title</span>
            <input
          ...
          {/* Terms — fixed height, scrolls WITHIN itself, does not resize/grow the modal */}
          <div className="flex flex-col gap-1">
            <span className="active">Terms</span>
            <textarea
              rows={7}
              placeholder="Paste/Write your terms and conditions here"
              style={{
                resize: "none", // 🔹 no more free resizing — that was what blew the modal out
                height: "180px", // 🔹 fixed height
                overflowY: "auto", // 🔹 scrolls internally once content exceeds the fixed height
              }}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#4CA1AF] transition-colors"
              {...register("Terms")}
            />
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-2 gap-y-2 gap-x-4 mt-2">
              {APPLICABLE_OPTIONS.map((opt) => (
                <div key={opt.key} className="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                  <input
                    type="checkbox"
                    className="w-4 h-4 cursor-pointer"
                    style={{ accentColor: ACCENT }}
        ...
        {/* Footer — pinned, never scrolls, always visible */}
