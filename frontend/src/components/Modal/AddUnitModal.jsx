...
// import { useAddItemUnitMutation } from "../../redux/api/itemApi";
...
import { useState } from "react";
import { toast } from "react-toastify";
import { useAddItemUnitMutation, useEditItemUnitMutation } from "../../redux/api/itemApi";

export default function AddUnitModal({ onClose, onSave, initialData = null }) {
  console.log("initialData", initialData);
  const isEditing = !!initialData?.id;
  console.log("isEditing", isEditing);

  const [itemUnitName, setItemUnitName] = useState(initialData?.Unit_Name || "");
  const [itemUnitShortHand, setItemUnitShortHand] = useState(initialData?.Unit_Shorthand || "");

  const [addItemUnit, { isLoading: isAddingItemUnitLoading }] = useAddItemUnitMutation();
  const [editItemUnit, { isLoading: isEditingItemUnitLoading }] = useEditItemUnitMutation();

  const isSaving = isAddingItemUnitLoading || isEditingItemUnitLoading;

  const handleSave = async () => {
    if (isSaving) return; // 🔥 STOP DOUBLE CALL
    try {
      if (!itemUnitName) {
        toast.error("Unit Name cannot be empty!");
        return;
      ...
      if (isEditing) {
        ...
        //onClose();
        onSave({
          id: initialData.id, // 🔹 edit response typically doesn't echo the id back — reuse the one we already have
          Unit_Name: payload.Unit_Name,
          Unit_Shorthand: payload.Unit_Shorthand,
        });
      } else {
        ...
        //onClose();
        onSave({
          id: res?.id ?? res?.data?.id, // 🔹 adjust to whatever your addItemUnit response actually returns
          Unit_Name: payload.Unit_Name,
          Unit_Shorthand: payload.Unit_Shorthand,
        });
      }
      ...
      toast.error(
        err?.data?.message ||
          (isEditing ? "Updating Item Unit Failed!" : "Adding Item Unit Failed!")
      );
    }
  };
