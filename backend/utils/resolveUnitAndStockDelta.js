...
export function resolveUnitAndStockDelta({
  ...
  const primaryUnit = dbItemRow?.Primary_Unit || null;
  const secondaryUnit = dbItemRow?.Secondary_Unit || null;
  const conversionRate =
    Number(dbItemRow?.Conversion_Rate) || 0;

  const chosenUnit = Selected_Unit || primaryUnit || null;

  // No configured primary unit
  if (!primaryUnit) {
    return {
      stockDelta: qty,

      snapshot: {
        Primary_Unit_Snapshot: chosenUnit,
        Secondary_Unit_Snapshot: null,
      },
  ...
  // Selected secondary unit
  if (chosenUnit === secondaryUnit) {
    if (conversionRate <= 0) {
      throw new Error(
        `Item has an invalid conversion rate for ${chosenUnit}.`
      );
    ...
    return {
      ...
      snapshot: {
        Primary_Unit_Snapshot: primaryUnit,
        Secondary_Unit_Snapshot: chosenUnit,
      },
  ...
  // Custom/editable unit
  // Accept it instead of throwing "must be Plt"
  return {
    stockDelta: qty,
