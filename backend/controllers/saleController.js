...
const buildLegacyInvoiceNumber = (
  invoiceNumberPrefix,
  invoiceNumberValue
) => {
  const prefix =
    String(invoiceNumberPrefix ?? "").trim() || "None";
  ...
  // 0 / 00 / 000000 means no invoice number
  if (number === 0) {
    return null;
  }
...
const getSingleSale = async (req, res, next) => {
  ...
  try {
    ...
    const [saleData] = await connection.query(
     ...
     s.Billing_Address,
     s.Invoice_Number,
     s.Invoice_Number_Prefix,
     s.Invoice_Number_Value,
     s.Invoice_Date,
     s.State_Of_Supply,
     s.Total_Amount,
     s.Round_Off,
  ...
  const hasHistoricalMRP =it.MRP !== null && Number(it.MRP) > 0;
    ...
    return res.status(200).json({
      ...
      invoicePartyDetails: {
        ...
        Invoice_Number_Prefix: saleHeader.Invoice_Number_Prefix,
        Invoice_Number_Value: saleHeader.Invoice_Number_Value,
...
//         const requestedPrefixName = String(
//             requestedPrefix
//         ).trim();
...
//         // =========================================================
//         // 2. VERIFY PREFIX EXISTS FOR SALE
//         // =========================================================
...
//                   AND TRIM(Invoice_Number) REGEXP '^[0-9]+$'
//                 ORDER BY
//                     CAST(TRIM(Invoice_Number) AS UNSIGNED) DESC
//                 LIMIT 1
//                 `,
//                 [activeFY]
//             );
...
//             if (rows.length > 0) {
//                 const lastInvoiceNumber = String(
...
//         // AEPL-2627-0087 -> 0088
//         //
//         // Only AEPL-2627- uses 4 digit numbering.
//         // =========================================================

//         const prefix = requestedPrefixName;
...
//             FROM add_sale
//             WHERE Financial_Year = ?
//               AND TRIM(Invoice_Number) LIKE CONCAT(?, '%')
//               AND SUBSTRING(
//                     TRIM(Invoice_Number),
//                     CHAR_LENGTH(?) + 1
//                   ) REGEXP '^[0-9]+$'
...
//             const numericPart = lastInvoiceNumber.substring(
//                 prefix.length
//             );
...
//                 // =================================================
//                 // AEPL-2627- IS THE OLD SPECIAL FORMAT
//                 // =================================================

//                 if (
//                     prefix.toUpperCase() === "AEPL-2627-"
//                 ) {
//                     newInvoiceNumber = String(
//                         nextNumber
//                     ).padStart(4, "0");
