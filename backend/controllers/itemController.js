  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Service items cannot have stock details.",
      });
    }
    ...
    */
    ...
    `,
      [
        ...
        normalizeNumber(Discount_On_MRP_For_Sale) || null,
        Sale_Price ?? null,
...
const editItem = async (req, res, next) => {
  ...
  try {
    ...
    console.log("EDIT ITEM VALIDATED DATA:", {
      Item_Id,
      MRP,
      Discount_On_MRP_For_Sale,
    });
    ...
    console.log("MASTER UPDATE VALUES:", {
      Item_Id,
      MRP,
      normalizedMRP: normalizeNumber(MRP),
      finalMRP: normalizeNumber(MRP) || null,
      Discount_On_MRP_For_Sale,
      finalDiscount: normalizeNumber(Discount_On_MRP_For_Sale) || null,
    });
    ...
    */
    ...
    return res.status(200).json({
      ...
      item: {
        ...
        Discount_On_MRP_For_Sale: Discount_On_MRP_For_Sale ?? null,
  ...
  `,
    ...
    if (Number(ledgerCount.count) > 0) {
      ...
      return res.status(400).json({
        ...
        canDelete: false,
        message:
          "This item cannot be deleted because transactions exist for this item. Delete related purchases, sales, returns, or stock adjustments first.",
      });
    }

    // DELETE OPENING STOCK ENTRIES
    await connection.query(
      `
...
const getItemsForDropdown = async (req, res, next) => {
  let connection;
  ...
  try {
    ...
    if (search) {
      ...
      whereClauses.push(`
        (
          LOWER(ai.Item_Name) LIKE LOWER(?)
          OR LOWER(ai.Item_Code) LIKE LOWER(?)
          OR LOWER(ai.Item_Id) LIKE LOWER(?)
          OR LOWER(ai.Item_Category) LIKE LOWER(?)
          OR LOWER(ai.Item_HSN) LIKE LOWER(?)
        )
      `);
...
LIMIT ?
  `,
    ...
    // =====================================================
    // GET ONLY THE ACTUAL PAGE ITEMS
    // AND FORMAT THEM
    // =====================================================
    ...
    console.error(
      "❌ Error fetching items for dropdown:",
      err
    );
...
const getItemByCode = async (req, res, next) => {
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({
        success: false,
        message: `No item found for code "${trimmedCode}".`,
      });
    }
...
const getItemBills = async (req, res, next) => {
  ...
  `,
    ...
    const filteredTransactions = isService
      ? transactions.filter(
        (row) =>
          ![
            "Opening_Stock",
    ...
    return res.status(200).json({
      ...
      transactions: filteredTransactions.map((row) => {
        let resolvedBillId = row.Bill_Id;

        if (
          row.Txn_Type === "Purchase_Return" &&
          row.Source_Id
        ) {
          resolvedBillId =
            purchaseReturnMap.get(row.Source_Id) ??
            null;

        } else if (
          row.Txn_Type === "Sale_Return" &&
          row.Source_Id
        ) {
          resolvedBillId =
            saleReturnMap.get(row.Source_Id) ??
            null;
        }
        ...
        return {
          ...
          Selected_Unit:
            adjustment?.Selected_Unit ??
            row.Selected_Unit ??
            null,
          //Selected_Unit: row.Selected_Unit,

          Document_Id: resolvedBillId,
    ...
    //     Conversion_Rate:
    //       item.Conversion_Rate !== null? Number(item.Conversion_Rate): null,
...
export {
  ...
  getAllItemsForLedger, getItemByCode, getItemBills, getItemsByCategory, addStockAdjustment, editStockAdjustment,
