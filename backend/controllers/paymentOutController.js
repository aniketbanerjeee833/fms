...
CREATE TABLE IF NOT EXISTS payment_out (
  Payment_Out_Id    VARCHAR(20)     NOT NULL PRIMARY KEY,
...
// const generatePaymentOutId = async (connection) => {
//   const [[row]] = await connection.query(
//     `SELECT Payment_Out_Id FROM payment_out ORDER BY created_at DESC LIMIT 1`
//   );
//   if (!row) return "POUT001";
//   const num = parseInt(row.Payment_Out_Id.replace("POUT", ""), 10) + 1;
//   return `POUT${String(num).padStart(3, "0")}`;
// };
...
const createPaymentOut = async (req, res, next) => {
  ...
  try {
    ...
    // const Payment_Out_Id = await generatePaymentOutId(connection);

    await connection.query(
      `INSERT INTO payment_out
         (Party_Id, Receipt_No, Payment_Date, Payment_Type, Paid)
       VALUES ( ?, ?, ?, ?, ?)`,
      [ Party_Id, Receipt_No || null, Payment_Date, Payment_Type, Number(Paid)]
    );

    return res.status(201).json({ success: true, message: "Payment Out created", Payment_Out_Id });
  } catch (err) {
    next(err);
  } finally {
    if (connection) connection.release();
...
const updatePaymentOut = async (req, res, next) => {
  ...
  try {
    connection = await db.getConnection();
    const { id } = req.params;
    const { Party_Id, Receipt_No, Payment_Date, Payment_Type, Paid, Notes } = req.body;
    ...
    await connection.query(
      `UPDATE payment_out
       SET Party_Id = ?, Receipt_No = ?, Payment_Date = ?,
           Payment_Type = ?, Paid = ?
       WHERE id = ?`,
      [Party_Id, Receipt_No || null, Payment_Date, Payment_Type, Number(Paid),  id]
    );
