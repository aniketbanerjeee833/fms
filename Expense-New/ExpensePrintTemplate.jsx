...
const getTaxRate = (taxType = "") => {
  ...
  const match = String(taxType).match(/(?:GST|IGST)(0(?:\.25)?|3|5|12|18|28)$/);
...
const ExpensePrintTemplate = forwardRef(({ expense }, ref) => {
  ...
  const showDiscountColumn = useMemo(() => {
    return items.some((item) => num(item?.Discount_Amount) > 0);
  }, [items]);
...
======================================================= */
  ...
  const taxSummary = useMemo(() => {
    ...
    summary.rates = Object.values(taxGroups).sort((a, b) => a.rate - b.rate);
  ...
  const totalQuantity = useMemo(() => {
    return items.reduce((sum, item) => sum + num(item?.Quantity), 0);
  }, [items]);
  ...
  const transactionDiscountPercentage = num(
    expense?.Transaction_Discount_Percentage,
  );

  const transactionDiscountAmount = num(expense?.Transaction_Discount_Amount);

  const showDiscount = transactionDiscountAmount > 0;
...
===================================================== */}
      ...
      <div className="expense-company-header">
        ...
        <div className="expense-logo">
          <img src="/assets/images/anco_logo.png" alt="ANCO Innovation" />
        </div>

        {/* COMPANY DETAILS */}
        <div className="expense-company-details">
          <div className="expense-company-name">{companyName}</div>
      ...
      <table className="expense-table">
        <thead>
          <tr>
            <th className="expense-section-header">Expense For</th>

            <th className="expense-section-header-right">Expense Details</th>
          </tr>
        </thead>
        ...
        <tbody>
          <tr>
            ...
            <td className="expense-cell">
              {expense?.Party_Name && (
                <div className="expense-bold">{expense.Party_Name}</div>
              )}

              {expense?.Billing_Address && <div>{expense.Billing_Address}</div>}

              {expense?.Phone_Number && (
                <div>Contact No. : {expense.Phone_Number}</div>
              )}

              {expense?.GSTIN && <div>GSTIN : {expense.GSTIN}</div>}

              {expense?.State && <div>State: {expense.State}</div>}
            </td>
            ...
            <td className="expense-cell-right">
              {expense?.Expense_Number && (
                <div>
                  <strong>Expense No. :</strong> {expense.Expense_Number}
                </div>
              )}
              ...
              {expense?.Bill_Date && (
                <div>
                  <strong>Bill Date :</strong> {formatDate(expense.Bill_Date)}
                </div>
              )}

              {expense?.State_Of_Supply && (
                <div>
                  <strong>Place of Supply :</strong> {expense.State_Of_Supply}
                </div>
              )}
            </td>
          </tr>
      ...
      <table className="expense-table expense-items">
        <thead>
          <tr>
            <th className="expense-table-header">#</th>

            <th className="expense-table-header">Item name</th>

            {showHsnColumn && <th className="expense-table-header">HSN/SAC</th>}

            <th className="expense-table-header">Quantity</th>

            <th className="expense-table-header">Price/Unit</th>

            {showDiscountColumn && (
              <th className="expense-table-header">Discount</th>
            )}

            {showTax && (
              <th className="expense-table-header">Taxable Amount</th>
            )}

            {showTax && showCGSTSGST && (
              <>
                <th className="expense-table-header">CGST</th>

                <th className="expense-table-header">SGST</th>
              </>
            )}

            {showTax && showIGST && (
              <th className="expense-table-header">IGST</th>
            )}
        ...
        <tbody>
          {items.length > 0 ? (
            items.map((item, index) => {
              ...
              const taxableAmount = Math.max(0, amount - taxAmount);

              const taxRate = getTaxRate(item?.Tax_Type);

              const cgstAmount = isGST(item?.Tax_Type) ? taxAmount / 2 : 0;

              const sgstAmount = isGST(item?.Tax_Type) ? taxAmount / 2 : 0;

              const igstAmount = isIGST(item?.Tax_Type) ? taxAmount : 0;
              ...
              return (
                <tr key={item?.id || index}>
                  <td className="expense-item-center">{index + 1}</td>
                  ...
                  <td className="expense-item-right">₹ {money(price)}</td>

                  {showDiscountColumn && (
                    <td className="expense-item-right">{discountText}</td>
                  )}
                  ...
                  <td className="expense-item-right">₹ {money(amount)}</td>
                </tr>
              );
            })
          ) : (
          ...
          {items.length > 0 && (
            <tr>
              ...
              {/* Item name */}
              <td className="expense-total-cell" style={{ textAlign: "left" }}>
                Total
              </td>

              {/* HSN/SAC */}
              {showHsnColumn && <td className="expense-total-cell"></td>}

              {/* Quantity */}
              <td className="expense-total-cell">{money(totalQuantity)}</td>
              ...
              {/* Discount */}
              {showDiscountColumn && (
                <td className="expense-total-cell">₹ {money(totalDiscount)}</td>
              )}
              ...
              {/* Amount */}
              <td className="expense-total-cell">₹ {money(totalAmount)}</td>
            </tr>
          )}
        </tbody>
      </table>
...
===================================================== */}
        ...
        {showTax ? (
          <div className="expense-bottom-left">
            <table className="expense-summary-table">
              <thead>
                <tr>
                  <td className="expense-summary-cell">Tax Details</td>
              ...
              <tbody>
                {showCGSTSGST && (
                  <>
                    <tr>
                      <td className="expense-summary-cell">CGST</td>
                      ...
                      {taxSummary.rates.map((group) => (
                        <td
                          ...
                          className="expense-summary-cell-right"
                        >
                          {group.type === "GST" ? `₹ ${money(group.cgst)}` : ""}
                        </td>
                      ))}
                    </tr>

                    <tr>
                      <td className="expense-summary-cell">SGST</td>
                      ...
                      {taxSummary.rates.map((group) => (
                        <td
                          ...
                          className="expense-summary-cell-right"
                        >
                          {group.type === "GST" ? `₹ ${money(group.sgst)}` : ""}
                        </td>
                      ))}
                    </tr>
                  </>
                ...
                {showIGST && (
                  <tr>
                    <td className="expense-summary-cell">IGST</td>
                    ...
                    {taxSummary.rates.map((group) => (
                      <td
                        ...
                        className="expense-summary-cell-right"
                      >
                        {group.type === "IGST" ? `₹ ${money(group.igst)}` : ""}
                      </td>
                    ))}
                  </tr>
                )}
    ...
    =================================================== */}
        ...
        <div className="expense-bottom-right">
          <div className="expense-summary-header">Amounts</div>

          <table className="expense-summary-table">
            <tbody>
              <tr>
                <td className="expense-summary-cell">Sub Total</td>
                ...
                <tr>
                  <td className="expense-summary-cell">
                    ...
                    {transactionDiscountPercentage > 0
                      ? `(${transactionDiscountPercentage.toFixed(2)}%)`
                      : ""}
                  </td>

                  <td className="expense-summary-cell-right">
                    ₹ {money(transactionDiscountAmount)}
                  </td>
                </tr>
              )}
              ...
              {showRoundOff && (
                <tr>
                  <td className="expense-summary-cell">Round off</td>
              ...
              <tr>
                <td className="expense-summary-cell">
                  <div className="expense-bold">Total</div>

                  <div>Paid</div>
                </td>

                <td className="expense-summary-cell-right">
                  <div className="expense-bold">₹ {money(totalAmount)}</div>
              ...
              <tr>
                <td className="expense-summary-cell">Balance</td>
...
===================================================== */}
      ...
      <div className="expense-bottom-grid">
        <div className="expense-bottom-left">
          <div className="expense-words-header">Amount in words</div>
      ...
      <div className="expense-signature">
        <div className="expense-signature-company">For : {companyName}</div>

        <div className="expense-signature-authorized">Authorized Signatory</div>
      </div>
    </div>
  );
});
