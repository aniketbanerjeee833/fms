...
export default function EditExpense() {
  ...
  const [editExpense, { isLoading: isUpdatingExpense }] =
    useEditExpenseMutation();
  ...
  useEffect(() => {
    ...
    reset({
      ...
      Round_Off:
        expense.Round_Off !== null && expense.Round_Off !== undefined
      ...
      items: (() => {
        ...
        return (
          expense.items?.map((item) => ({
            ...
            Discount_Type_On_Price: item.Discount_Type_On_Price || "Percentage",
    ...
    // Split box
    // -----------------------------------------
    setShowSplitBox(expense.splits && expense.splits.length > 1);
  }, [expenseResponse, reset, enableTransactionWiseDiscount]);
  ...
  const getUsedIdentifiers = (excludeIndex) =>
    splitsValues
      .map((s, i) =>
        i === excludeIndex
          ? null
          : getRowIdentifier(s.Payment_Type, s.Bank_Account_Id),
      )
      .filter(Boolean);
               ...
               Show None + original saved tax type */
                              ...
                              originalTaxTypesRef.current[i] !== "None" && (
                                <option value={originalTaxTypesRef.current[i]}>
                                  {originalTaxTypesRef.current[i].startsWith(
                                    "GST",
                                  )
                                    ? `GST@${originalTaxTypesRef.current[
                                        i
                                      ].replace("GST", "")}%`
                                    : originalTaxTypesRef.current[i].startsWith(
                                          "IGST",
                                        )
                                      ? `IGST@${originalTaxTypesRef.current[
                                          i
                                        ].replace("IGST", "")}%`
                                      : originalTaxTypesRef.current[i]}
                                </option>
