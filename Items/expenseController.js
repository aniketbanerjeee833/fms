...
const getExpenseItemUsage = async (req, res, next) => {
  ...
  try {
    ...
    const limit = 10; // matches your PAGE_SIZE

    const whereClauses = [`ei.Expense_Item_Master_Id = ?`];
    const params = [masterItemId];

    // =========================================================
    // CURSOR — compound { date, id }, same pattern as party ledger
    // =========================================================
    ...
    // =========================================================
    // SEARCH — Exp No. / Party / Amount
    // (Payment_Type is resolved AFTER the query via payment_splits,
    //  so it can't be searched in this WHERE clause directly —
    ...
    const [rows] = await connection.query(
      `SELECT ei.id, ei.Quantity, ei.Price, ei.Amount,
       ...
       LEFT JOIN add_party a ON e.Party_Id = a.Party_Id
       WHERE ${whereClauses.join(" AND ")}
       ORDER BY e.Expense_Date DESC, ei.id DESC
       LIMIT ?`,
      [...params, limit + 1]
    );
    ...
    // =========================================================
    // ATTACH Payment_Type_Display — unchanged logic
    // =========================================================
    ...
    // =========================================================
    // NEXT CURSOR — compound, matches party ledger pattern
    // =========================================================
    ...
    if (hasMore && pageRows.length > 0) {
      ...
      nextCursor = Buffer.from(
        JSON.stringify({ date: last.Expense_Date, id: last.id })
      ).toString("base64");
    }
